...
ITERATIONS = int(os.getenv("PASSWORD_HASH_ITERATIONS", "300000"))
...
def verify_password(password: str, stored: str) -> bool:
    try:
        ...
        got = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), bytes.fromhex(salt), ITERATIONS).hex()
...
def current_producer(request: Request):
    ...
    c = get_db()
    try:
        return c.execute("SELECT * FROM producers WHERE id=?", (pid,)).fetchone()
    finally:
        c.close()
